...
void CorrectionTask::InitializeQA(std::string detectorName, DetectorType type)
{
	...
	if (type == DetectorType::Track)
	{
		...
		h2 -> push_back (new TH2F (Form ("h2centMult%s", detectorName.c_str ()),  Form ("mult:cent (%s);centrality class;subevent multiplicity", detectorName.c_str ()), 6, 0, 6, 500, 0, 500));
