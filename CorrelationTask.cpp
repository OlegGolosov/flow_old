...
//  a.AddEventVariable({"Eveto", eVetoBins});
  a.AddEventVariable({"Centrality", {10, 0, 100}});
	
//  a.AddEventVariable({"Multiplicity", multBins10});
//  a.AddFunction("TPC", Rebin);

  const std::string sDet[] = { "PSD1", "PSD2", "PSD3", "TPC_pt", "TPC_y", "STS"/*, "TPC_1", "TPC_2", "TPC_3", "TPC_a_1", "TPC_b_1"*/};
  const std::string sMc[] = { "MC_pt", "MC_y"};
...
// end RS 2 harmonics
  ...
  for (ushort iPsd=0; iPsd<3; ++iPsd)
    for (ushort iTrack=0; iTrack<3; ++iTrack)
    {
  ...
  for (ushort iDet=0; iDet<3; ++iDet)
  {
    ...
    a.AddCorrelation("STS_" + sPsdPsd[iDet] + "_X2XX", "STS, " + sPsdPsdName[iDet], X2XX);
    a.AddCorrelation("STS_" + sPsdPsd[iDet] + "_X2YY", "STS, " + sPsdPsdName[iDet], X2YY);
    a.AddCorrelation("STS_" + sPsdPsd[iDet] + "_Y2XY", "STS, " + sPsdPsdName[iDet], Y2XY);
    a.AddCorrelation("STS_" + sPsdPsd[iDet] + "_Y2YX", "STS, " + sPsdPsdName[iDet], Y2YX);
