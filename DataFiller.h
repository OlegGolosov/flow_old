...
class DataFiller {
 ...
 public:
  ...
  {
    DataFiller::event_ = event;
		DataFiller::hist1 = &hist1;
		DataFiller::hist2 = &hist2;
		DataFiller::pid_getter = pid_getter;
...
//		int size = DataFiller::hist2 -> at ("TPC_1") -> size ();
  ...
  void FillMCTrackingDetector(std::map<std::string, Detector> &detectors, const DataTreeEvent &event, std::string detectorName) const;
